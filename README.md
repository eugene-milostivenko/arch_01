...
- Межсервисных вызовов, где требуется подтверждение выполнения (например, Automation Executor Service → Device Management для отправки команды на устройство)
...
- Потребления событий Telemetry Service (для сохранения) и Automation Trigger Service (для оценки триггеров)
